...
The release artefact is core with the generated column terms inserted after the end of
its MS: block -- immediately before the PEFF: block -- so the merged file stays in
ascending id order within each prefix (core's MS ids end at MS:4000215, the fragment's
begin at MS:5000000).

This is a pure text insertion: core's bytes either side of the seam are untouched, and
the fragment is copied verbatim. That makes the build deterministic and the released
psi-ms.obo byte-stable, so a release diff shows only real content changes -- which is
the whole reason the release is built this way rather than round-tripped through ROBOT.
ROBOT still generates psi-ms.owl *from* this file, one-way, exactly as upstream does.
